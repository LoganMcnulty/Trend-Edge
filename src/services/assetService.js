...
export function getTrendEdgeHistorical(assetNameList, settings){
  const {fastSMA, slowSMA, lookback, fastWeight, slowWeight, fastOverSlowWeight, macdWeight, adxWeight} = settings
  let assetString = ''
  assetNameList.forEach(asset => {
    assetString += `${asset},`
  })
  const url = `${apiEndpoint}/trendEdgeHistorical/${assetString}/${fastSMA}/${slowSMA}/${lookback}/${fastWeight}/${slowWeight}/${fastOverSlowWeight}/${macdWeight}/${adxWeight}`
  return http.get(url).then(res => res)
}
